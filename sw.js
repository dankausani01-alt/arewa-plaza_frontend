...
// Cached one-by-one (not cache.addAll) so a single missing/renamed file
// can't silently prevent every other asset from being cached — addAll()
// is all-or-nothing and rejects the whole batch on any one failure.
      ...
      return Promise.all(
        STATIC_ASSETS.map((url) =>
          cache.add(url).catch((err) => {
            console.warn('[SW] Failed to cache (skipped, others unaffected):', url, err);
          })
        )
      );
    })
